...
function catcommands.effects.hobble(user, target, notify, status, data)
	...
	if notify then
		core.chat_send_player(target:get_player_name(), message)
	end
end
...
core.register_on_joinplayer(function(target)
...
function catcommands.utility.proclaim(message)
	...
	if core.get_modpath("irc") and irc.connected and irc.config.send_join_part then
		irc:say(message)
	end
end